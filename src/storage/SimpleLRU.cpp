...
bool SimpleLRU::Delete(const std::string &key)
{
    ...
    size_t size = key.size() + buff->value.size();
    if(buff == _lru_head.get())
    {
        _lru_head.reset(buff->prev);
        if(_lru_head.get() != nullptr)
        {
            _lru_head.get()->next.reset(nullptr);
        }
    }
    else
    {
...
bool SimpleLRU::_Delete_tail_node()
{
    ...
    size = buff->key.size() + buff->value.size();
